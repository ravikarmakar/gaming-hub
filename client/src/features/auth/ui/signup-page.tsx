...
import { signupSchema, SignupSchemaType, useRegisterMutation } from "@/features/auth";
...
export default function SignupPage() {
  ...
  const { mutateAsync: register, isPending: isLoading, error } = useRegisterMutation();
  ...
  const { login: loginWithGoogle, isLoading: isGoogleLoading } = useGoogleAuth();
  const { login: handleDiscordLogin, isLoading: isDiscordLoading } = useDiscordAuth();
  ...
  const handleSubmit = async (values: SignupSchemaType) => {
    ...
    if (result?.user) {
      ...
      startTransition(() => {
        ...
        if (!result.user!.isAccountVerified) {
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-2" noValidate>
          ...
          <motion.div variants={itemVariants} className="mt-2">
            <SocialLogin
              ...
              isDiscordLoading={isDiscordLoading}
            />
          </motion.div>
