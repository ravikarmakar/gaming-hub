...
import { TeamTournament } from "../../lib/types";
import { MemberHeader } from "../components/roster/MemberHeader";
import { TournamentItem } from "../components/dashboard/TournamentItem";
import { useTeamDashboard } from "../../context/TeamDashboardContext";
import { useTeamTournamentsQuery } from "../../hooks/useTeamQueries";


const TeamTournamentsPage = () => {
  const { team: currentTeam, teamId } = useTeamDashboard();

  const { data: teamTournaments = [], isLoading } = useTeamTournamentsQuery(teamId || "", {
    enabled: !!teamId
  });
  ...
  const filteredTournaments = teamTournaments.filter((t: TeamTournament) => {
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto w-full px-4 md:px-6 pt-2 md:pt-4 pb-4 md:pb-8 space-y-8">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTournaments.map((tournament: TeamTournament) => (
              <TournamentItem
                key={tournament._id}
                id={tournament._id}
                ...
                date={tournament.startDate ? new Date(tournament.startDate).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric'
                }) : 'TBD'}
