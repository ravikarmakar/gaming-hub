...
import { useUpdateMessageMutation } from "../../hooks/useChatMutations";
import { ChatHistoryResponse, ChatMessage } from "../../api/chatApi";
...
export const MessageInput = ({ onSendMessage, isLoading }: MessageInputProps) => {
  ...
  const onSubmit = async (data: MessageValues) => {
    ...
    try {
      if (editingMessage) {
        const queryKey = chatKeys.history(editingMessage.targetId, editingMessage.scope);
        const previousHistory = queryClient.getQueryData(queryKey);

        // Optimistically update
        queryClient.setQueryData<ChatHistoryResponse>(queryKey, (old) => {
          if (!old) return old;
          return {
            ...old,
            messages: old.messages.map((m: ChatMessage) =>
              m._id === editingMessage._id ? { ...m, content: trimmedContent, isEdited: true } : m
            ),
          };
        });
        ...
        try {
          await updateMessageMutation.mutateAsync({
            targetId: editingMessage.targetId,
            messageId: editingMessage._id,
            scope: editingMessage.scope,
          ...
          // Do NOT clear editingMessage so user can retry
          throw mutationError;
        }
      } else {
    ...
    >
      <div className="relative w-full px-2 md:px-4">
        ...
        <div className={cn(
          ...
          (isSubmitting || isLoading) && "border-purple-500/40 bg-[#1A1625]",
          ...
          <Textarea
            ...
            rows={1}
            className={cn(
              "flex-1 bg-transparent border-none text-white placeholder:text-gray-500 focus-visible:ring-0 focus-visible:ring-offset-0 py-1.5 px-3 text-sm resize-none scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent selection:bg-purple-500/30",
              (isLoading || isSubmitting) && "opacity-80 cursor-default"
            )}
          />
