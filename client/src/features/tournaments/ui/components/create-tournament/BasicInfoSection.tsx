        ...
        <GlassCard className="p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        <Gamepad2 className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 z-10" size={16} />
                        <Controller
                            name="game"
                            control={control}
                            render={({ field }) => (
                                <Select value={field.value} onValueChange={field.onChange}>
                                    <SelectTrigger className="bg-white/5 border-white/10 h-12 pl-12 rounded-xl focus:border-purple-500/50 transition-all font-bold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div className="space-y-2">
                    ...
                    <Select value={watch("category")} onValueChange={(val) => setValue("category", val as any, { shouldValidate: true, shouldDirty: true })}>
                ...
                <div className="space-y-2">
                    ...
                    <Select value={watch("eventType")} onValueChange={(val) => setValue("eventType", val as any, { shouldValidate: true, shouldDirty: true })}>
                        <SelectTrigger className="bg-white/5 border-white/10 h-12 rounded-xl">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="bg-[#0B0C1A] border-white/10 text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div className="space-y-2">
                    <Label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Tournament Mode</Label>
                    <Select value={watch("registrationMode")} onValueChange={(val) => setValue("registrationMode", val as any, { shouldValidate: true, shouldDirty: true })}>
                        <SelectTrigger className="bg-white/5 border-white/10 h-12 rounded-xl focus:border-purple-500/50 transition-all font-bold">
                            <SelectValue placeholder="Select registration mode" />
                        </SelectTrigger>
                        <SelectContent className="bg-[#0B0C1A] border-white/10 text-white">
                            {registrationModeOptions.map((option) => (
                                <SelectItem key={option.value} value={option.value} className="focus:bg-purple-500/10 focus:text-purple-400">
                                    {option.label}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    {errors.registrationMode && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.registrationMode.message}</p>}
                </div>

                <div className="space-y-2">
                    <Label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Number of Matches</Label>
                    <Input
                        ...
                        className="bg-white/5 border-white/10 h-12 rounded-xl focus:border-purple-500/50 transition-all font-bold"
                    />
                    {errors.matchCount && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.matchCount.message}</p>}
                </div>
                ...
                <div className="col-span-full space-y-4 pt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {(() => {
                            ...
                            if (availableMaps.length === 0) {
                                return <p className="text-[10px] text-gray-400 italic">No predefined maps for this game.</p>
                            }
                                    ...
                                    <div className="space-y-3 w-full">
                                        <div className="flex flex-wrap gap-2">
                                            {availableMaps.map((mapName: string) => (
                                                <span
                                                    key={mapName}
                                                    className="px-4 py-2 rounded-xl text-xs font-bold bg-white/5 border border-white/10 text-gray-400"
                                                >
                                                    {mapName}
                                                </span>
                                            ))}
                                        </div>
                                        <p className="text-[10px] text-purple-400 font-bold italic animate-pulse">
                                            * Map rotation will be assigned randomly from these maps for each match in scrims.
                                        </p>
                                    </div>
                                );
                            }
                    ...
                    {errors.map && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.map.message}</p>}
                </div>
            </div>
