...
import Group from "../event/models/group.model.js";
...
class ChatService {
  ...
  initializeHandlers() {
    ...
    this.registerScopeHandler("group", {
      checkAccess: async (userId, targetId) => {
        const groupInfo = await Group.findById(targetId).select("teams roundId").lean();
        if (!groupInfo) return false;

        // 1. Check if user is in one of the teams
        const isTeamMember = await Team.exists({
          _id: { $in: groupInfo.teams },
          $or: [{ captain: userId }, { "teamMembers.user": userId }],
          isDeleted: false
        });
        ...
        // 2. Not a team member? Load complex relations for Organizer access
        const populatedGroup = await Group.findById(targetId)
          .select("roundId")
          .populate({
            path: "roundId",
            select: "eventId",
        ...
        const orgId = populatedGroup?.roundId?.eventId?.orgId;
      ...
      getSenderRole: async (userId, targetId) => {
        ...
        const populatedGroup = await Group.findById(targetId)
          ...
          .populate({
            path: "roundId",
            select: "eventId",
            populate: { path: "eventId", select: "orgId" }
          })
          .lean();
    ...
    // User (Private) Scope Handler
    this.registerScopeHandler("user", {
      checkAccess: async (userId, targetId) => {
        // Anyone can attempt to message another user, but usually we verify they both exist
        return await User.exists({ _id: targetId, isDeleted: false });
      },
      getSenderRole: async () => "user"
    });
  }
   ...
   */
  async getHistory(scope, targetId, { limit = 50, before = null, userId = null } = {}) {
    // Upper bound validation to prevent resource exhaustion
    const finalLimit = Math.min(Math.max(parseInt(limit) || 50, 1), 100);
    let query;
    
    // For private user-to-user chat, we must have a symmetric query to show the full conversation
    if (scope === "user") {
      if (!userId) {
        throw new CustomError("Authentication required for private chat history", 400);
      }
      query = {
        scope: "user",
        isDeleted: false,
        $or: [
          { sender: userId, targetId: targetId },
          { sender: targetId, targetId: userId }
        ]
      };
    } else {
      query = { targetId, scope, isDeleted: false };
    }
    ...
    return {
      ...
      hasMore: messages.length === finalLimit
    };
  }
  ...
  /**
   * Invalidate cached access for a specific user and target
   * Useful when a user is kicked or role changes
   */
  async invalidateAccessCache(scope, userId, targetId) {
    try {
      await redis.del(`chat_access:${scope}:${userId}:${targetId}`);
      await redis.del(`chat_role:${scope}:${userId}:${targetId}`);
    } catch (err) {
      logger.error(`Redis error clearing access cache: ${err.message}`);
    }
  }
   ...
   */
  async updateMessage(messageId, userId, content, targetId) {
    if (!content || typeof content !== "string" || content.trim().length === 0) {
      throw new CustomError("Message content is required", 400);
    }

    const message = await Chat.findOne({ _id: messageId, isDeleted: false });
    if (!message) {
      throw new CustomError("Message not found", 404);
    }

    const hasAccess = await this.checkAccess(message.scope, userId, message.targetId);
    if (!hasAccess) {
      throw new CustomError("Message not found", 404); // Obfuscate to prevent side-channel
    }

    if (message.targetId.toString() !== targetId.toString()) {
      throw new CustomError("Target ID mismatch", 400);
    }
   ...
   */
  async deleteMessage(messageId, userId, targetId) {
    ...
    const hasAccess = await this.checkAccess(message.scope, userId, message.targetId);
    if (!hasAccess) {
      throw new CustomError("Message not found", 404); // Obfuscate
    }
    ...
    if (!canDelete) {
      const senderRole = await this.getSenderRole(message.scope, userId, message.targetId);
      if (["owner", "manager", "staff", "official"].includes(senderRole)) {
        canDelete = true;
      }
    }

    if (!canDelete) {
      throw new CustomError("You are not authorized to delete this message", 403);
    }
